 ...
 *
 * A React effect writing to localStorage fails hardest for the traffic this
 * funnel buys: in the Instagram and Facebook in-app browsers a visitor can tap
 * the CTA and navigate away before hydration runs the capture, and those same
 * browsers restrict the storage it writes to. Either way the campaign is gone
 * by checkout and the buyer reaches fulfilment with blank columns.
 *
 * Reading the query string HERE, on the first request, removes the race.
 * lib/attribution.ts stays as the redundant second copy, and both writers
 * share one cookie and one shape so they agree rather than overwrite.
 */
