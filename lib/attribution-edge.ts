 ...
 *
 * THIS FILE HAS NO `'use client'` DIRECTIVE AND MUST NOT GAIN ONE. Everything
 * here runs where there is no browser: middleware.ts at the edge, and the API
 * routes. Its sibling lib/attribution.ts IS marked `'use client'`, so it
 * cannot be called from either: the import succeeds and the call throws, which
 * once returned a 500 from a live checkout. The dependency runs one way, the
 * browser file importing this one, never back.
 */

export const KEY = 'rn_attr';
...
/* Capped at CAPTURE, not at send. These values ride to the webhook inside
   Razorpay note values, which Razorpay caps at 256 characters, and a landing
   url with five utm params and an fbclid routinely runs past 400. */
export const CAP = {
  utm: 100,
  fbclid: 200,
  referrer: 200,
...
/** Tolerant of both wire forms: the value Next has already decoded once, and a
 *  still-encoded one. Raw is tried FIRST, because decoding first would corrupt
 ...
 * differently, and getting this backwards is how a funnel reports every sale
 * as coming from its own landing page:
 ...
 * and guaranteed under 256 characters.
 *
